  ...
  const buildTableRows = () => {
    ...
    for (let i = 0; i < maxRows; i++) {
      ...
      const getDataForDay = (day: number | undefined) => {
        ...
        const morningDisplay = morningQty > 0 ? `${parseFloat(morningQty.toFixed(2))}L` : '';
        const eveningDisplay = eveningQty > 0 ? `${parseFloat(eveningQty.toFixed(2))}L` : '';
        const groceryDisplay = groceryTotal > 0 ? `₹${parseFloat(groceryTotal.toFixed(2))}` : '';
              ...
              <div className="border-t-2 border-gray-300 pt-3 sm:pt-4">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 text-center">
                  <div className="bg-blue-50 p-2 sm:p-3 rounded">
                    ...
                    <p className="text-sm sm:text-lg font-bold text-blue-600">{parseFloat(totalMilk.toFixed(2))}L</p>
                  </div>
                  <div className="bg-green-50 p-2 sm:p-3 rounded">
                    <p className="text-xs text-gray-600">Milk Amount</p>
                    <p className="text-sm sm:text-lg font-bold text-green-600">₹{parseFloat(totalMilkAmount.toFixed(2))}</p>
                  </div>
                  <div className="bg-purple-50 p-2 sm:p-3 rounded">
                    <p className="text-xs text-gray-600">Grocery</p>
                    <p className="text-sm sm:text-lg font-bold text-purple-600">₹{parseFloat(totalGroceryAmount.toFixed(2))}</p>
                  </div>
                  <div className="bg-yellow-50 p-2 sm:p-3 rounded border-2 border-yellow-400">
                    <p className="text-xs text-gray-600">Grand Total</p>
                    <p className="text-base sm:text-xl font-bold text-orange-600">₹{parseFloat(grandTotal.toFixed(2))}</p>
                  </div>
                </div>
                ...
                <div className="mt-3 text-center">
                  <div className="inline-block bg-red-50 px-3 sm:px-4 py-2 rounded border-2 border-red-200">
                    <p className="text-xs sm:text-sm text-gray-600">
                      Previous Balance: <span className="text-red-600 font-bold">₹{parseFloat(pendingBalance.toFixed(2))}</span>
                    </p>
                  </div>
                </div>
                ...
                {monthlyPayments > 0 && (
                  <div className="mt-3 text-center">
                    <div className="inline-block bg-green-50 px-3 sm:px-4 py-2 rounded border-2 border-green-200">
                      <p className="text-xs sm:text-sm text-gray-600">
                        Payment Received: <span className="text-green-700 font-bold">₹{parseFloat(monthlyPayments.toFixed(2))}</span>
                      </p>
                    </div>
                  </div>
                )}
                  ...
                  <div className="mt-3 text-center">
                    <div className="inline-block bg-yellow-50 px-3 sm:px-4 py-2 rounded border-2 border-yellow-400">
                      <p className="text-xs sm:text-sm text-gray-800 font-semibold">
                        Pending After Payment: <span className="text-orange-600 font-bold">₹{parseFloat(pendingAfterPayment.toFixed(2))}</span>
                      </p>
                    </div>
                  </div>
                )}
